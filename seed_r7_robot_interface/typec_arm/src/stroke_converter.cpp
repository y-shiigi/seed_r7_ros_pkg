...
void seed_converter::TypeCArm::makeTables()
{
...
void seed_converter::TypeCArm::Angle2Stroke
(std::vector<int16_t>& _strokes, const std::vector<double>& _angles)
{
  ...
  _strokes[9] = static_cast<int16_t>(scale * (rad2Deg * _angles[9] + 50.0) * 0.18);

  _strokes[10]
    = static_cast<int16_t>(scale * setAngleToStroke(- rad2Deg * _angles[10], leg.table));  //knee
  _strokes[11]
    = static_cast<int16_t>(scale * setAngleToStroke(  rad2Deg * _angles[11], leg.table));  //ankle
}

void seed_converter::TypeCArm::Stroke2Angle
(std::vector<double>& _angles, const std::vector<int16_t>& _strokes)
{
  ...
  _angles[6] = deg2Rad * (scale_inv * _strokes[9] * 5.556 - 50.0);
  _angles[7] = 0;
  _angles[8] = 0;
  _angles[9] = -deg2Rad * (scale_inv * _strokes[9] * 5.556 - 50.0);

  _angles[10] = -deg2Rad * setStrokeToAngle(scale_inv * _strokes[10], leg.inv_table);  // knee
  _angles[11] = deg2Rad * setStrokeToAngle(scale_inv * _strokes[11], leg.inv_table);  // ankle

}

PLUGINLIB_EXPORT_CLASS(seed_converter::TypeCArm, seed_converter::StrokeConverter)
